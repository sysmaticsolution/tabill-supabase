...
export default function AuthCallback() {
  ...
  useEffect(() => {
    ...
    const routeForUser = async (authUser: any) => {
      if (!authUser) return;
    ...
    const tryHandleSession = async () => {
      ...
      if (session?.user) {
        handled = true;
        await routeForUser(session.user);
      }
    };
    ...
    // Initial attempt and a short retry to avoid race conditions
    void tryHandleSession();
    const shortRetry = setTimeout(() => { void tryHandleSession(); }, 250);

    // Fallback if nothing happens within 8s
    const failTimer = setTimeout(() => {
      if (!handled) router.push('/login?error=auth_callback_failed');
    }, 8000);
